(function () {
  ...
  const headerHandle = document.querySelector("[data-header-handle]");
  ...
  // Reveal animation (never hides content if JS fails)
...
${message}`;
  ...
  if (headerHandle) {
    headerHandle.addEventListener("click", () => {
      if (!header) return;
      header.classList.contains("is-visible") ? hideHeader() : showHeader();
    });
  }

  // Desktop top-edge hover reveal
  let topEdgeTimer = null;
  document.addEventListener("mousemove", (e) => {
    if (window.innerWidth < 980) return;
    if (e.clientY <= 18) {
      showHeader();
      if (topEdgeTimer) clearTimeout(topEdgeTimer);
      topEdgeTimer = setTimeout(() => {
        if (!navMenu || !navMenu.classList.contains("is-open")) hideHeader();
      }, 2400);
    }
  });

  // Click outside header to hide (when menu closed)
  document.addEventListener("click", (e) => {
    if (!header || !header.classList.contains("is-visible")) return;
    const inside = header.contains(e.target) || (headerHandle && headerHandle.contains(e.target));
    if (!inside && (!navMenu || !navMenu.classList.contains("is-open"))) hideHeader();
  });

  // Hidden even at very top:
  hideHeader();
})();
